    ...
    <>
      ...
      <section className="max-w-[1200px] mx-auto px-12 py-10">
        ...
        <div className="flex gap-1.5 flex-wrap mb-2 border-b-2 border-[--bdr] pb-0">
          ...
          {SERVICES.map(s => {
            const hasLanding = ['implantologie','protetica','allon','digital-checkup','consultatie-online'].includes(s.slug)
            const landingUrl = s.slug === 'implantologie' ? '/servicii/implant-dentar' : s.slug === 'protetica' ? '/servicii/coroane-dentare' : s.slug === 'allon' ? '/servicii/dinti-ficsi-all-on' : s.slug === 'digital-checkup' ? '/digital-checkup' : s.slug === 'consultatie-online' ? '/consultatie-online' : ''
            return hasLanding ? (
        ...
        {!activeTab ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-6">
            {SERVICES.map((svc, index) => {
              const iconPath = SVC_ICONS[svc.slug]
              const landingUrl = svc.slug === 'implantologie' ? '/servicii/implant-dentar' : svc.slug === 'protetica' ? '/servicii/coroane-dentare' : svc.slug === 'allon' ? '/servicii/dinti-ficsi-all-on' : svc.slug === 'digital-checkup' ? '/digital-checkup' : svc.slug === 'consultatie-online' ? '/consultatie-online' : ''
              return (
                <Card
                  key={svc.slug}
